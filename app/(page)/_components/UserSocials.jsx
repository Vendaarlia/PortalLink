...
export default function UserSocials({ userDataName }) {
    ...
    return (
        <div className="wrapper">
            <div className="mt-10 px-2 md:px-20 lg:px-32 grid place-content-center">
                ...
                <div onClick={() => { navigator.share({ url: location.href, title: name, text: name }) }} className="bg-[--ijo] text-[--black] w-12 h-12 rounded-full flex items-center justify-center fixed top-5 right-5 md:top-10 md:right-32 cursor-pointer hover:scale-95 transition backdrop-blur-3xl">
                ...
                <div className="grid place-content-center mb-14 text-center gap-1 px-2">
                    ...
                    <p className="[font-size: var(--clamp-txt)] max-w-[320px]">{bio}</p>
                    {loding && (
                ...
                {!loding && (
                    <div className="grid gap-3 grid-cols-1 max-w-[600px]">
                        {!youtube ? null : (
                            <Link className="transition md:w-[420px] w-[300px] mx-auto h-14 flex items-center justify-center rounded-full hover:bg-[--ijo] hover:text-black hover:scale-95 relative" target="_blank" href={youtube}><Youtube className="absolute left-6 h-5 w-5" /> My YouTube</Link>
                        )}
                        {!insta ? null : (
                            <Link className="transition md:w-[420px] w-[300px] mx-auto h-14 flex items-center justify-center rounded-full hover:bg-[--ijo] hover:text-black hover:scale-95 relative" target="_blank" href={insta}><Instagram className="absolute left-6 h-5 w-5" /> My Instagram</Link>
                        )}
                        {!face ? null : (
                            <Link className="transition md:w-[420px] w-[300px] mx-auto h-14 flex items-center justify-center rounded-full hover:bg-[--ijo] hover:text-black hover:scale-95 relative" target="_blank" href={face}><Facebook className="absolute left-6 h-5 w-5" /> My Facebook</Link>
                        )}
                        {!github ? null : (
